...
obs %>% 
  ggplot(aes(MDE1,MDE2))+
  scattermore::geom_scattermore(aes(color=pct_counts_mt))+
  scale_color_viridis()+
  labs(color='',title='% counts MT')+
  theme_void()+theme(legend.position = 'none',plot.title = element_text(hjust = 0.5, size=12))-> pl
pl
ggsave(plot=pl,filename = '../figures/S3/A-mt.png',width=3,height=3, dpi="retina")
...
dat.pl = obs %>% filter(clone_id!='',subset=='CD8') %>% 
  left_join(read.csv('../resources/tcell_labels2.csv') %>% select(pheno,labels2)) %>% 
  mutate(pheno=labels2) %>% 
...
obs %>% filter(clone_id %in% mait_tcr) %>% group_by(pheno) %>% tally() %>% 
  mutate(pct=n/sum(n)*100) %>% arrange(-pct) %>% print()
```
...
exp %>% left_join(obs %>% select(obs_names,pheno,donor_id)) %>% 
  ...
  mutate(outlier = ifelse(donor_id==donor.Tex_hi,'Outlier donor','Others')) %>% 
  ggplot(aes(outlier,value,fill=outlier))+facet_grid(~name)+
  geom_violin()+geom_boxplot(alpha=0,show.legend=F)+
  scale_fill_manual(values=c('Outlier donor'='#666669',Others='grey80'))+
  ggpubr::stat_compare_means(comparisons = list(c('Outlier donor','Others')),
  method = 'wilcox.test',size=3.5)+
  labs(y='Scaled expression', fill='')+
  theme_classic()+
  theme(
    legend.position = 'bottom',
...
pl
ggsave(plot=pl,filename = '../figures/S3/G.pdf',width=4,height=2, dpi="retina")
