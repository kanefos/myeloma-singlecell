...
obs.pl = valid_obs %>% mutate(pheno_pred = factor(pheno_pred, unique(names(Tcell_pheno_colors))) )
...
comp$valid %>% 
  ...
  select(source,diagnosis,celltype,clr,prop) %>% 
  ...
  filter(source_diag %in% c('atlas_Non','atlas_MM','Botta2023_MM','atlas_SMM','Botta2023_SMM')) %>% 
  ...
  filter(celltype %in% c('CD8.TemTerm','CD8+Tte')) %>% 
  
  filter(diagnosis %in% c('Non','SMM','MM')) -> dat
