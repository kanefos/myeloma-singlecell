...
pl
ggsave(plot=pl,filename = '../figures/S5/A-right.pdf',width=6,height=3.5, dpi="retina")
...
results$Invar$cohort$addAge$Non.MM %>% 
  mutate(fdr = p.adjust(p.cohort,'BH')) %>% filter(fdr < 0.1)
results$Invar$cohort$addAge$Non.SMM %>% 
  mutate(fdr = p.adjust(p.cohort,'BH')) %>% filter(fdr < 0.1)

results$Treg$cohort$addAge$Non.MM %>% 
  mutate(fdr = p.adjust(p.cohort,'BH')) %>% filter(fdr < 0.1)
results$Treg$cohort$addAge$Non.SMM %>% 
  mutate(fdr = p.adjust(p.cohort,'BH')) %>% filter(fdr < 0.1)
...
results$Invar$cohort$addAge$Non.SMM %>% 
  ...
  geom_vline(xintercept = 0, linewidth=0.2)+
  geom_point(color='lightgrey',alpha=0.8)+
  geom_point(data = . %>% filter(sig,change=='down'), color=disease_col['Non'])+
  geom_point(data = . %>% filter(sig,change=='up'), color=disease_col['SMM'])+
  ggrepel::geom_text_repel(data = . %>% filter(sig), aes(label=celltype), size=3)+
  labs(x='Δ median', y='-log10 FDR', title='Non v SMM')+
  xlim(c(-2.5,2.5))+ylim(c(0,8))+
  theme_classic()+theme(
    plot.title = element_text(hjust = 0.5, size=12),
    axis.title.x = element_text(size=13), axis.text.x = element_text(size=11),
...
pl
ggsave(plot=pl,filename = '../figures/S5/subclus-invarSMM.pdf',width=3.5,height=3, dpi="retina",device=cairo_pdf)
...
results$Tcell$risk$risk_ISS %>% 
  mutate(fdr = p.adjust(p.risk_ISS,'BH')) %>% 
  mutate(sig=fdr<0.1, change=ifelse(diff>0,'up','down')) %>% 
  ggplot(aes(diff, -log10(fdr)))+
  geom_hline(yintercept = -log10(0.1), linetype='dotted', linewidth=0.5)+
  geom_vline(xintercept = 0, linewidth=0.2)+
  geom_point(color='lightgrey',alpha=0.8)+
  geom_point(data = . %>% filter(sig), color=disease_col['MM'])+
  ggrepel::geom_text_repel(data = . %>% filter(sig), aes(label=celltype), size=3)+
  labs(x='Δ median', y='-log10 FDR', title='ISS risk')+
  #xlim(c(-2.5,2.5))+#ylim(c(0,8))+
  theme_classic()+theme(
    plot.title = element_text(hjust = 0.5, size=12),
    axis.title.x = element_text(size=13), axis.text.x = element_text(size=11),
...
pl
ggsave(plot=pl,filename = '../figures/S5/E-1.pdf',width=3.5,height=3, dpi="retina",device=cairo_pdf)


results$Tcell$risk$risk_SMM_mayo %>% 
  mutate(fdr = p.adjust(p.risk_SMM_mayo,'BH')) %>% 
  mutate(sig=fdr<0.1, change=ifelse(diff>0,'up','down')) %>% 
  ggplot(aes(diff, -log10(fdr)))+
  geom_hline(yintercept = -log10(0.1), linetype='dotted', linewidth=0.5)+
  geom_vline(xintercept = 0, linewidth=0.2)+
  ...
  geom_point(data = . %>% filter(sig), color=disease_col['MM'])+
  ggrepel::geom_text_repel(data = . %>% filter(sig), aes(label=celltype), size=3)+
  labs(x='Δ median', y='-log10 FDR', title='SMM risk (Mayo)')+
  #xlim(c(-2.5,2.5))+#ylim(c(0,8))+
  theme_classic()+theme(
    plot.title = element_text(hjust = 0.5, size=12),
    axis.title.x = element_text(size=13), axis.text.x = element_text(size=11),
...
pl
ggsave(plot=pl,filename = '../figures/S5/E-2.pdf',width=3.5,height=3, dpi="retina",device=cairo_pdf)
...
pc1.pct = round((ords$Tcell$CA$eig / sum(ords$Tcell$CA$eig) * 100)['PC1'],1)
pc2.pct = round((ords$Tcell$CA$eig / sum(ords$Tcell$CA$eig) * 100)['PC2'],1)
...
data.frame(i=round((ords$Tcell$CA$eig / sum(ords$Tcell$CA$eig) * 100),1)) %>% 
  rownames_to_column('PC') %>% 
  head(5) %>% mutate(v='v') %>% 
  arrange(-i) %>% mutate(PC=factor(PC,PC)) %>% 
  ggplot(aes(PC,i))+
  geom_line(aes(group=v))+geom_point()+
  labs(y='% variance\nexplained')+
  theme(
