...
pct_pheno = obs %>% filter(!lineage %in% c('Doublet')) %>% 
  ...
  group_by(covar,lineage) %>% tally() %>% ungroup() %>% 
  group_by(covar) %>% mutate(pct=n/sum(n)) %>% ungroup() %>% 
  mutate(covar = factor(covar,c('nPB','nBM','cPB','cBM')))

pct_pheno %>% mutate(pct=pct*100) %>% 
  ggplot(aes(covar,pct))+geom_bar(stat='identity',width=1.1, aes(fill=lineage))+
...
pl
ggsave(plot=pl,filename = '../figures/1/1E.1.pdf',width=1.5,height=2.5, dpi="retina")
